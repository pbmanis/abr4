...
% modified so that HW.STIM.block allows rapid multiple reps to be collected in
% sequence.
% the number of reps in a block is HW.STIM.np, the interval between reps is
...
if HW.STIM.StimPerSweep == 1
    nRecordPoints = floor(HW.STIM.avg_dur*HW.STIM.sample_freq/1000.);
else
    nRecordPoints = floor((HW.STIM.StimPerSweep*HW.STIM.ipi/1000)*HW.STIM.sample_freq); % convert from msec to sec, samples/second
end
nStimPoints = floor((HW.STIM.StimPerSweep*HW.STIM.ipi/1000)*HW.STIM.NIFreq);
InterpFreq = 100000;

timebase_Stim = 0:1/HW.STIM.NIFreq:(nStimPoints-1)/HW.STIM.NIFreq; % express rate in msec
timebase_Record = 0:(1/HW.STIM.sample_freq):(nRecordPoints-1)/HW.STIM.sample_freq;
stimBlockLen = floor(HW.STIM.ipi/1000*HW.STIM.NIFreq);
recBlockLen = floor(HW.STIM.ipi/1000.*HW.STIM.sample_freq);
interBlockLen = floor(HW.STIM.ipi/1000.*InterpFreq);
timebase_Interpolate = 0:(1/InterpFreq):((HW.STIM.StimPerSweep*interBlockLen-1)/InterpFreq);
timebase_Display = 1000*(0:(1/InterpFreq):(interBlockLen-1)/InterpFreq);

replot = 0;
t0 = 1;
t1 = floor(1e-3*0.5*HW.STIM.sample_freq);
t2 = 1;
t3 = floor(1e-3*2.0*HW.STIM.sample_freq);
tbs = 1;
tbe = floor(1e-3*0.9*HW.STIM.sample_freq);

tp0 = floor(1e-3*2.0*HW.STIM.sample_freq);
tp1 = floor(1e-3*6.0*HW.STIM.sample_freq);
...
% repeatedly.
if(HW.STIM.Alternate == 0)
    stdur = HW.STIM.ipi;
    stdacrepeat = HW.STIM.StimPerSweep;
    stpmax = floor((stdur*10^-3)/(1/HW.STIM.NIFreq));
    aosamples = length(HW.STIM.wave);
else
    stdur = 2.0*HW.STIM.ipi;
    stdacrepeat = ceil(HW.STIM.StimPerSweep/2);
    stpmax = floor((stdur*10^-3)/(1/HW.STIM.NIFreq));
    aosamples = length(HW.STIM.wave);
end
if ~isempty(AO) % only do this if we are using NI
    ...
    swave = HW.STIM.wave(1:stpmax);
else
    swave = HW.STIM.wave;
end
scaleFactor_ch1 = 1.0/HW.STIM.amp_gain; % (10/(2^23))*1000000/HW.STIM.amp_gain; % convert to microvolts, correcting for a/d in rp2.1
...
set_status('Running');
plotSweep = cell(3, HW.STIM.NSweeps);

for i = 1:HW.STIM.NSweeps % loop over all the sweeps.
    sweepdata = zeros(4, interBlockLen);
    %    fprintf(2,'sweep: %d   stdacrepeat: %d \n', i, stdacrepeat);
    HW.STIM.Monitor = get_SignalPlotFlag(2); % allow plot flag to change when running
    IN_ACQ = 1; %#ok<*NASGU>
    ...
    if ~isempty(AO)
        ...
        set_attn(local_attn);
        AO.stop 
        AO.Rate = HW.STIM.NIFreq; 
        queueOutputData(AO, HW.STIM.wave); % wave is FULL
        AO.TriggersPerRun = 1; % set(AO, 'repeatoutput',  1);
        set_status('Running');
        tic
        startBackground(AO); % get ni board read to go, then trigger the rp
        err = rp_setup(HW, nRecordPoints+1000, 'start');
        if err == 1
            fprintf(2, 'failed to start\n');
            return;
        end
        recdur = nRecordPoints/HW.STIM.sample_freq;
        ...
        recordblocking(ar, nRecordPoints/HW.STIM.sample_freq)
        ch1 = getaudiodata(ar);
    end
    IN_ACQ = 0;
    ...
    ch1r = reshape(ch1i(1:(interBlockLen*HW.STIM.StimPerSweep)), ...
        [interBlockLen, HW.STIM.StimPerSweep]);
    ...
    if(HW.STIM.Monitor == 1)
        ...
        ch2r = reshape(ch2i(1:(interBlockLen*HW.STIM.StimPerSweep)), ...
            [interBlockLen, HW.STIM.StimPerSweep]);
        ch2m = mean(ch2r, 2);
...
%         end;
    ...
    iy = [];
    li = 1:HW.STIM.StimPerSweep;
    
    if HW.STIM.avg_reject > 0
        [~, iy] = find(abs(ch1r) > HW.STIM.avg_reject*10^-6);
        allstds=std(ch1r(tbs:tbe,:));
        w=mean(allstds);
        iy2 = find(allstds > w*HW.STIM.rms_reject)';
        ...
        if(HW.STIM.Alternate)
            switch(mod(ii,2)) % put alternate data into different data sets.
                case 1
                    ...
                    end % input channel 2 is microphone monitor of HW.STIM...
                    Sweep_np = Sweep_np + 1;
                case 0
    ...
    if(~isempty(HW.STIM.hrep))
        set(HW.STIM.hrep, 'string', sprintf('%d', i*HW.STIM.StimPerSweep)); % update acquisition trial counter
    end
    if(mod(replot, 10) == 0)
        replot = 0;
    else
...
%     end;
    set(PLOTHANDLES.data, 'xlim', [0 HW.STIM.avg_dur]);
    set(PLOTHANDLES.data, 'clipping', 'on');
    drawnow;
    
    % fprintf(1, 'Ch2 max in sweepdata: %f, nonalt: %d\n', max(sweepdata(3,:)), Sweep_nonalt);
    if(HW.STIM.Monitor == 1) % only do this if we have the monitor set on
        hold off
    ...
    % delay to next sweep:
    if i < HW.STIM.NSweeps % (but not on last sweep)
        while toc < HW.STIM.InterSweepInterval
            stf = check_stop(0);
...
data(1,:) = data(1,:)/HW.STIM.NSweeps;
data(2,:) = data(2,:)/HW.STIM.NSweeps;
if HW.STIM.Monitor
    data(3,:) = data(3,:)/HW.STIM.NSweeps;
    data(4,:) = data(4,:)/HW.STIM.NSweeps;
end

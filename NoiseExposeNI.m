...
topDBSPL = 123.5; % dB SPL, Crown Channel 1 at at 10 (green arrow), 0 dB Attn
...
if(nargin == 0) % initialize
    ...
    totalDuration = 0;
    total_elapsed = 0;
    open([pwd '\NoiseExposeNIFig.fig']); % get the figure window
    figurehandle = gcf;
    ...
    hattn = findobj('Tag', 'SelectAttn');
%    if(~isempty(hattn) && ishandle(hattn))
%        set(hattn, 'String', UseAttn);
%    end
    ...
    dbspl = topDBSPL - Attn;
    hdb = findobj('Tag', 'calc_dbspl');
    if(~isempty(hdb) && ishandle(hdb))
        set(hdb, 'String', sprintf('%.1f', dbspl));
    end
    % access the NI Card
    daqreset;
...
switch(varargin{1})
    ...
    case {'preview', 'noisecalc'}
        ...
        plot_spectrum(swave, fs, PlotHandles.spectrum, 1);
    ...
    case 'checklevel' % read the microphone and check the level/output
        ...
        M = load(sprintf('microphone_7016#10252.cal'), '-mat'); % 1/4 " MICROPHONE, get most recent calibration
        MIC = M.MIC;
        [swave, fs] = noise_gen('bandpass', Freq-(Band_Width/2.0), Freq+(Band_Width/2.0), 'butter', 4,1);
        [~, ch2, AIsamp, ~] = checklevel(swave, fs);
        mics = 1/AIsamp;
        tfs = 0:fs:(length(ch2)*fs - fs);
        tmic = 0:mics:(length(ch2)*mics)-mics;
        ch2s = interp1(tmic, ch2, tfs);
        plot_spectrum(ch2s, fs, PlotHandles.spectrum, 1)
        
        rmsval = rms(ch2);
        dbspl = MIC.RefSig + 20.*log10(rmsval/MIC.Vrms);
        % dbspl_spec = MIC.RefSig + 20*log10(2*Hpsd.Data/MIC.Vref); %#ok<NODEF>
        hdb = findobj('Tag', 'meas_dbspl');
        if(~isempty(hdb) && ishandle(hdb))
            set(hdb, 'String', sprintf('%7.1f', dbspl));
        end
        
        tb = 0:1.0/AIsamp:1./AIsamp*(length(ch2)-1);
        plot(PlotHandles.waveform, tb, ch2);
        
    case 'start' % --- Executes on button press in Start.
        % get most recent microphone calibration value
        M = load(sprintf('microphone_7016#10252.cal'), '-mat'); % 1/4 " MICROPHONE, get most recent calibration
        MIC = M.MIC;
        dscale = 3600;
        ...
        while QUIT == 0
            ...
            if last_dbsplcheck < (elapsed - 10)
                ...
                ctime = datetime(now,'ConvertFrom','datenum');
                fprintf(1, 'measured dBSPL: %7.1f  at %s\n', dbspl, ctime);
                last_dbsplcheck = elapsed;
            end
...
function plot_spectrum(swave, fs,  plothandle, yscale)
%
% swave is the 1-d waveform for spectrum calculation
% fs is the sample frequency
% plothandle is the plot where this will go
% yscale is either 1 or 2. If 1, we set the ylim for
% the stimulus calculation
% if 2, we let the ylim vary to show the full data range.

    global Freq
    global Band_Width
    [Pxx, F] = pwelch(swave, 8192, 1024, 1./fs);
    freqs = (F/pi)*0.5/fs;
    hold(plothandle, 'off')
    plot(plothandle, freqs, Pxx, 'k-');
    hold(plothandle, 'on')
    plot(plothandle, [Freq, Freq], [1e-6, 1e2], 'b-');
    plot(plothandle, [Freq-(Band_Width/2.0), Freq-(Band_Width/2.0)], [1e-6, 1e2], 'm-');
    plot(plothandle, [Freq+(Band_Width/2.0), Freq+(Band_Width/2.0)], [1e-6, 1e2], 'm-');
    hold(plothandle,  'off')
%        Hpsd = pwelch(swave,'Fs', 1.0/fs);
%        plot(plothandle, Hpsd.Frequencies(2:end)/1000., ...
%            Hpsd.Data(2:end));
    set(plothandle, 'XScale', 'log')
    set(plothandle, 'YScale', 'log')
    set(plothandle, 'XLim', [1e3 1e5]);
    if yscale == 1
        set(plothandle, 'YLim', [1e-6 1e2]);
    end
    set(plothandle, 'XTick', [1000, 2000, 5000, 10000, 20000, 50000, 100000]);
    set(plothandle, 'XTickLabel', {'1.0', '2.0', '5.0', '10.0', '20.0', '50.0', '100.0'});
    xlabel('kHz');
end     
%-------------------------------------------------------------------
...
if(~isempty(hattn))
    set(hattn, 'String', sprintf('%3d', attn_id));
end
...
function [ch1, ch2, AISamp, err] = read_mic()
%% record signal and print SPL and time
%% assumes stimulus waveform is already being played in AO
%% Uses RP2.1
%% 5/26/2023 Paul B. Manis
%%
global RP MicIn
